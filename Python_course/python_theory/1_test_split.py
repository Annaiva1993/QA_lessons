...
try:
    ...
    print(parts)
    a = int(parts[2]) #достаем 1ое число ()
    b = int(parts[4].replace('?','')) #достаем 2ое число, без учета знака ? и пробела
    ...
    driver.find_element(By.ID,'answer').send_keys(str(correct_answer)) #находим поле инпут, которому присваиваем значение correct_answer
